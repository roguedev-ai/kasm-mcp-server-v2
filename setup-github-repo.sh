...
# Setup script for creating GitHub repository for kasm-mcp-server

echo "==================================="
echo "Kasm MCP Server GitHub Setup"
echo "==================================="
echo ""

# Check if we're in the right directory
if [ ! -f "package.json" ] || [ ! -d "src" ]; then
    echo "Error: This script must be run from the kasm-mcp-server directory"
    exit 1
fi
...
echo "   - Repository name: kasm-mcp-server"
...
echo "   git remote add origin https://github.com/YOUR_USERNAME/kasm-mcp-server.git"
...
if command -v gh &> /dev/null; then
    ...
    echo "gh repo create kasm-mcp-server --public --source=. --remote=origin --push"
    echo ""
else
